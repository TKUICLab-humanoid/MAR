...
from tku_msgs.msg import YoloData
...
class EdgeTPUModel:
    ...
    def __init__(self, model_file, names_file, conf_thresh=0.25, iou_thresh=0.45, filter_classes=None, agnostic_nms=False, max_det=1000):
        ...
        """
        # rospy.init_node('highest',anonymous=True)
        self.Yolo_Highest_pub = rospy.Publisher('highest/msg',YoloData , queue_size=100)
        self.RosYolo = YoloData()
    ...
    def process_predictions(self, det, output_image, pad, output_path="detection.jpg", save_img=False, save_txt=False, hide_labels=False, hide_conf=False):
        ...
        if len(det):
            ...
            # Write results
            highest_target = [None for i in range(6)]
            highest_target[0] = 0
            for *xyxy, conf, cls in reversed(det):
                if save_img:  # Add bbox to image
                    c = int(cls)  # integer class
                    label = None if hide_labels else (self.names[c] if hide_conf else f'{self.names[c]} {conf:.2f}')
                    if conf > highest_target[0]:
                        highest_target[0] = conf
                        highest_target[1] = self.names[c]
                        highest_target[2:] = xyxy
            ...
            self.RosYolo.Conf =  highest_target[0] 
            self.RosYolo.Label =  highest_target[1]
            self.RosYolo.XMin =  highest_target[2]
            self.RosYolo.YMin =  highest_target[3]       
            self.RosYolo.XMax =  highest_target[4]
            self.RosYolo.YMax =  highest_target[5]
            self.Yolo_Highest_pub.publish(self.RosYolo)
            if save_txt:
                output_txt = base+"txt"
